...
var GenericBoxView = Backbone.View.extend({
  ...
  constructor: function () {
    ...
    this.$el.on('click', '[data-action=exit]', function () {
      self.hide();
    });
  },
  show: function () {
...
var EventList_ItemView = Backbone.View.extend({
  ...
  prender: function () { /* This must always return this */
    ...
    this.$el.html(this.template({
      ...
      description: this.model.get('description').split(' ').slice(0,40).join(' '),
...
var EventAdderView = GenericBoxView.extend({
  ...
  el: $("#add-event"),
  initialize: function () {
    $("#add-event form").bind('submit', function (event) {
  ...
  render: function () {
    console.log('aoooooooooooooooo2')  }
});
